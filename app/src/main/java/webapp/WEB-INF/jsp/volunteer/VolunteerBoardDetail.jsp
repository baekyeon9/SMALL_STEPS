		...
		<div class="board-wrap padding-board">
			...
			<form action='boardUpdate' method="post">
				<div class="form-group">
					<label for='f-no'></label> <input id='f-no' type='hidden' name='no'
						class="form-control col-sm-1" value='${volunteerBoardDTO.no}'
						readonly>
				</div>
            ...
            <label for='f-content' class="col-sm-2 col-form-label"><b>내용</b></label> 
             <textarea name="content" id="f-content" cols="30" rows="8" class="form-control">${volunteerBoardDTO.content}</textarea>
              ...
              <!-- //파일첨부 -->
                    ...
                    <div class="mb-3 row">
>>>>>>> df44cef494cf35518d81d584e6fef9cfdd3041cf
...
<<<<<<< HEAD
				...
				<div class="form-group">
					...
					<input id='f-registeredDate' type="text" readonly
						class="form-control-plaintext"
						value="${volunteerBoardDTO.registeredDate}">
				</div>

				<div class="form-group">
					<label for='f-viewCount' class="col-sm-2 col-form-label"><b>조회수</b></label>
					<input id='f-viewCount' type="text" name="viewcount" readonly
						class="form-control-plaintext"
						value="${volunteerBoardDTO.viewCount}">
				</div>


				<div class="button-box">
<%-- 					<c:if
						test="${sessionScope.loginUser.id == volunteerBoardDTO.owner.id}"> --%>
						<button class="btn btn-primary">변경</button>
						<a href='boardDelete?no=${volunteerBoardDTO.no}'
							class="btn btn-primary">삭제</a>
<%-- 					</c:if> --%>
					<input type="button" class="btn btn-outline-primary nBtn"
						value="이전" onClick="history.go(-1)" role="button">
...
>>>>>>> df44cef494cf35518d81d584e6fef9cfdd3041cf
	...
	<div class="dap_lo">
		<div class="dap_lo2">
			<c:forEach items="${volunteerBoardCommentList}"
				var="volunteerBoardCommentDTO" varStatus="vs">
				<div>
					<b>${volunteerBoardCommentDTO.owner.id}</b>
				</div>
					...
					<div class="modal fade" id="myModalQuestionU${vs.index}"
						...
						<div class="modal-dialog">
							...
							<div class="modal-content">
								...
								<form action='commentUpdate?no=${volunteerBoardCommentDTO.no}'
									method="post">
									<div class="modal-body">
										<div class="class" id="id" style="display: none">
											<label for='f-volBoardNo'>게시글 번호</label> <input
												id='f-volBoardNo' type='text' name='volBoardNo'
												value='${volunteerBoardCommentDTO.volBoardNo}' readonly>
											<br>
										</div>
										<div class="class" id="id" style="display: none">
											<label for='f-no'>댓글 번호</label> <input id='f-no' type='text'
												name='no' value='${volunteerBoardCommentDTO.no}' readonly>
											<br>
										</div>
										<textarea id='f-content' name='commentContent' cols="40"
											rows="1" class="modal-body">${volunteerBoardCommentDTO.commentContent}</textarea>
										<div>
											<label for='f-owner'>작성자</label> <span id='f-owner'>${volunteerBoardCommentDTO.owner.id}</span><br>
										</div>
										<label for='f-registeredDate'>등록일</label> <span
											id='f-registeredDate'>${volunteerBoardCommentDTO.registeredDate}</span><br>
									</div>
									<button>수정완료</button>
									<br>
								</form>
...
<%-- 					</c:if> --%>
	...
	<form action='commentAdd?volBoardNo=${volunteerBoardDTO.no}'
		method="post">
		<div class="form" id="form" role="form">
			...
			<div class="my-3 p-3 bg-white rounded shadow-sm"
				...
				<div class="row">
					<div class="col-sm-10">
						<textarea type="content" name='commentContent' id="content"
							class="form-control" rows="3" placeholder="로그인 후 댓글을 입력해 주세요"></textarea>
					</div>
					<br>
